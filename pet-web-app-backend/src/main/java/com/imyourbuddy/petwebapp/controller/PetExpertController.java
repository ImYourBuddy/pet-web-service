...
public class PetExpertController {
    ...
    @PutMapping("/{userId}")
    @PreAuthorize("hasRole('EXPERT')")
    public PetExpert edit(@PathVariable(name = "userId") long userId,
                          @RequestBody @Valid PetExpert updatedExpert) throws ResourceNotFoundException {
        return service.edit(userId, updatedExpert);
    }
